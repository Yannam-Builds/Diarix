...
description: "Run Diarix as a headless server with a web UI using Docker"
---

## Overview

Diarix can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full Diarix UI is served directly from the backend.
...
For production deployments, put Diarix behind nginx or Caddy with TLS and authentication:

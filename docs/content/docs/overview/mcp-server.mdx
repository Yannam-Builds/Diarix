...
Diarix ships a built-in **Model Context Protocol** server so local AI
agents can call your Diarix install directly: speak text in a voice
profile, transcribe audio, and list captures or profiles. The server runs
inside the same process as the rest of Diarix and is mounted at `/mcp`
over Streamable HTTP.

Agent asks to speak → Diarix plays audio on your speakers → an on-screen
...
Diarix → Settings → MCP. The value is just an identifier for the
...
<Tabs items={["macOS", "Windows", "Linux"]}>
  ...
  ```json
  {
    "mcpServers": {
      "voicebox": {
        "command": "/Applications/Diarix.app/Contents/MacOS/voicebox-mcp",
  ...
  ```json
  {
    "mcpServers": {
      "voicebox": {
        "command": "C:\\Program Files\\Diarix\\voicebox-mcp.exe",
        "env": { "VOICEBOX_CLIENT_ID": "claude-desktop" }
      }
    }
  }
...
The shim waits up to 30 seconds for the Diarix backend to come up, then
proxies JSON-RPC from stdio over Streamable HTTP. Diarix must be running
...
<Steps>
  ...
  <Step title="Per-client binding">
    ...
    **Diarix → Settings → MCP**. Lets you pin Claude Code to Morgan,
...
Diarix → Settings → MCP shows one row per `client_id` Diarix has heard
...
silent background TTS is a trust hazard, so Diarix always shows what's
...
<Callout type="info">
  ...
  Diarix is running — the backend only listens while the desktop app is
...
- **Localhost only.** The server binds to `127.0.0.1`. If you ever point
  Diarix at a non-loopback interface (e.g. remote-mode over a trusted
...
- **No auth today.** Any process that can connect to your loopback can
  call MCP. That's the same trust boundary as the rest of Diarix's REST
...
- **Lifespan:** mounting FastMCP requires the `lifespan=` kwarg on
  ...
  with FastMCP's Streamable HTTP session manager. The Diarix app.py

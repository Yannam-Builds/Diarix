...
Diarix is the **open-source, local-first AI voice studio**. It closes the
voice I/O loop in both directions on one machine, with no cloud and no
accounts:

> Diarix is an independent fork built from
> [Voicebox](https://github.com/jamiepine/voicebox). The Voicebox project remains
> upstream; Diarix adds its expanded transcription catalog and product workflow
> while preserving upstream interfaces for compatibility.
...
- **Agents talk back** — any MCP-aware agent can call Diarix to speak in
...
Diarix also runs a full speech recognition and local LLM stack, shared

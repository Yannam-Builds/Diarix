...
a chord, talk, release — Diarix transcribes what you said with Whisper,
...
<Callout type="info">
  ...
  per-profile personality modes. It's the "input" half of Diarix's voice I/O
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and Diarix has Accessibility permission, the
...
Diarix ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, Diarix records whether each modifier is the left or right variant.
...
Once transcription finishes, Diarix can synthesize a native paste into
...
- Whisper loop hallucinations — Diarix strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated Diarix is blocked by Windows itself. Run Diarix elevated
...
A few cases where Diarix deliberately does *not* synthesize a paste:

- **Focus was inside Diarix** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-Diarix round-trip doesn't

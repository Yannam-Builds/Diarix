...
The model runs through the same backend split Diarix already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — Diarix runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  true })` and Diarix rewrites the text in character before speaking.

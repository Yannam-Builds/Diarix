---
title: "Auto-Updater"
description: "How Diarix automatic updates work"
---

## Overview

Diarix uses Tauri's built-in auto-updater to deliver signed updates to users. The system verifies updates cryptographically before installation.

## How It Works

When Diarix launches (in production Tauri builds only), it checks GitHub Releases for a `latest.json` manifest. If a newer version is available:
...
- CUDA server core: ~945 MB (versioned with each Diarix release)
...
This means most Diarix upgrades only re-download the ~945 MB server core, not the full ~2.5 GB bundle.
...
5. **Restart** — The Diarix server restarts and swaps in the CUDA backend.
